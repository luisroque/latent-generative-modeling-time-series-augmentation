...
class Visualizer:
    ...
    def __init__(
        self, dataset, n_versions=6, n_series=6, input_dir="./", transf_data="whole"
    ):
        ...
        for i in range(self.y_new.shape[0]):
            for j in range(self.n_series):
                if (i + 1) % 6 == 0:
                    ax[j].plot(self.y[:, j], label="original", color="darkorange")
                ax[j].plot(
                    self.y_new[i, 0, :, j], label=f"version {i}", color=colors[i]
                )
                ax[j].set_title(
                    f'Series {j}, [{"".join(list(transf[:, j].astype("<U1")))}]'
                )
        plt.legend()
        plt.show()
    ...
    def visualize_avg_distance_by_version(dict_transf_version_dist, title):
        df = pd.DataFrame.from_dict(
            {i: dict_transf_version_dist[i] for i in dict_transf_version_dist.keys()},
            orient="index",
        )
