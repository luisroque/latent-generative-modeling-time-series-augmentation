"""
Baseline VAE benchmark: a vanilla LSTM variational autoencoder.

A generic recurrent VAE treating each series as one sample: 2-layer LSTM
encoder / decoder, per-element MSE reconstruction averaged over the batch,
KL weighted by 0.1, and 500 training iterations of one random batch of 64
series each.

Like the TimeVAE benchmarks, it has no temporal latent structure and no
equivariance regularisation—it uses a single global latent vector per
series.
"""
...
class BaselineVAEGenerator(TimeSeriesGenerator):
    ...
    def _fit(self, data: np.ndarray) -> None:
        ...
        for epoch in range(self.epochs):
            ...
            if epoch % log_every == 0 or epoch == self.epochs - 1:
                print(
                    f"  [BaselineVAE] epoch {epoch+1}/{self.epochs}  "
